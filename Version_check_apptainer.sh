...
echo "pandas (qc-1.0.0.sif):           $(apptainer exec apptainer/qc-1.0.0.sif python -m pip show pandas | grep -i Version)"
...
echo "psadd (rmd-1.0.0.sif):              $(apptainer exec apptainer/rmd-1.0.0.sif Rscript --vanilla -e 'suppressPackageStartupMessages({if (requireNamespace("psadd", quietly=TRUE)) 
cat(as.character(utils::packageVersion("psadd")),"\n") else cat("NOT INSTALLED\n")})' 2>&1 | tail -n 1)"
