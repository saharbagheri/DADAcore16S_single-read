...
if(primer_removal== TRUE & primer_investigation==TRUE){
  object_names <- list.files(in_dir, pattern = forward_read_suffix,full.names = T)
} else {
  object_names <- list.files(in_dir, pattern = forward_read_suffix,full.names = T)
  object_names=object_names[!grepl(pattern = "primerRMV",x = object_names)]
  names<-unique(sapply(strsplit(object_names,split=forward_read_suffix),`[`,1))
}
...
object_names <- ls(pattern = forward_read_suffix)
...
colnames(reads1)[1:6]<-c("Sample", "Raw", "not-Quality_Trimmed", "not-Filtered", "Denoised", "Cleaned")
...
read_loss<-data.frame(step=names(colSums(reads1[,-c(1, 7)])),count=colSums(reads1[,-c(1, 7)]))

read_loss$step<- c("Raw", "Quality_Trimmed", "Filtered", "not-Denoised","Chimeric")
...
reads1[,7]<-data.frame(reads1$Cleaned/reads1$Raw)*100
colnames(reads1)[7]<-"Clean_Reads_Percent"
reads1[,7]<-round(reads1[,7],digits = 2)
